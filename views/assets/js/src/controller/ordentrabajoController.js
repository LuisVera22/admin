class OrdentrabajoController {
    ...
    bloqueCampos() {
        ...
        if (vendor) {
            $('#encabezado_orden #selectRecepcionado').attr('disabled', true);
            $('#encabezado_orden #selectSede').attr('disabled', true);
        }
    }
    generarIdentificadorUnico() {
        const self = this;
        let commonSession = 'id-' + Math.random().toString(36).substring(2, 9);
        if (!sessionStorage.getItem('tabID')) {
            sessionStorage.setItem('tabID', commonSession);
        } else {
            sessionStorage.removeItem(`selectedItems`);
            sessionStorage.removeItem(`selectedLaboratory`);
    ...
    mostrandoCuotas() {
        ...
        $('#selectFormaPago').change(function () {
            ...
            if (param === "CRÉDITO") {
                ...
                $('.cuadro_credito').hide();
                sessionStorage.removeItem(`selecteCuotas`);
                self.updateSecondTableCuotas();
            }
        })
    }
    mostrandoManufacturacion() {
        ...
        $('#agregando_item').click(function () {
            param = $('#encabezado_orden #selectTipoManufactura').val();
            if (param === "") {
    ...
    tablaProductos() {
        ...
        $('#cargar_producto').click(function () {
            ...
            let itemsLaboratorio = JSON.parse(sessionStorage.getItem(`selectedLaboratory`)) || [];
            ...
            sessionStorage.setItem(`selectedItems`, JSON.stringify(items));
            sessionStorage.setItem(`selectedLaboratory`, JSON.stringify(itemsLaboratorio));
            toastr.success('La base se ha agregado a la tabla. <i class="far fa-smile-wink"></i>');
            self.updateSecondTable();
        });
    ...
    tablaCuotas() {
        ...
        $('#agregando_cuotas').click(function () {
            ...
            let items = JSON.parse(sessionStorage.getItem(`selectedCuotas`)) || [];
            let total = $('#total').val();
        ...
        $('#frmRegistrarCuotas').submit(function (e) {
            ...
            if (fechaCuota <= emisionFecha) {
                ...
                if (montoExedido > totalPago) {
                    ...
                    toastr.success('La cuota se ha agregado a la tabla. <i class="far fa-smile-wink"></i>');
                    sessionStorage.setItem(`selectedCuotas`, JSON.stringify(items));
                    self.updateSecondTableCuotas();
                }
            }
    ...
    eliminarItems() {
        ...
        $('#secondTable').on('click', '.delete-row', function () {
            ...
            let items = JSON.parse(sessionStorage.getItem(`selectedItems`));
            let itemsLaboratory = JSON.parse(sessionStorage.getItem(`selectedLaboratory`)) || [];
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    sessionStorage.setItem(`selectedItems`, JSON.stringify(items));
                    sessionStorage.setItem(`selectedLaboratory`, JSON.stringify(itemsLaboratory));
                    self.updateSecondTable();
                } else {
    ...
    actualizarItemProducto() {
        ...
        $('#secondTable').on('click', '.update-row', function () {
            ...
            let selectedLaboratory = JSON.parse(sessionStorage.getItem(`selectedLaboratory`)) || [];
            let laboratoryItem = selectedLaboratory.find(lab => lab.commonId === selectedItem.commonId);
        ...
        $('#frmEditarItems #update_item').click(function () {
            ...
            sessionStorage.setItem(`selectedItems`, JSON.stringify(items));
            sessionStorage.setItem(`selectedLaboratory`, JSON.stringify(selectedLaboratory));
            toastr.info('Se ha actualizado los valores de la base. <i class="far fa-smile-wink"></i>');
            self.updateSecondTable();
    ...
    onCreateOrdenTrabajoFormSubmit() {
        $('#btnOrdenTrabajo').click(function () {
            ...
            if ($formapago === "CRÉDITO") {
                let $cuotas = JSON.parse(sessionStorage.getItem(`selectedCuotas`));
                if ($cuotas === null) {
            ...
            let $itemsCuotas = JSON.parse(sessionStorage.getItem(`selectedCuotas`));
            if ($itemsCuotas === null) {
                $itemsCuotas = [];
            ...
            Swal.fire({
                title: '<strong>REGISTRAR ORDEN DE TRABAJO</strong>',
                text: "Revisar antes de registrar!. Si todo es conforme hacer clic en 'Aceptar'",
                icon: 'info',
                showCancelButton: true,
                confirmButtonColor: '#3085d6',
                cancelButtonColor: '#d33',
